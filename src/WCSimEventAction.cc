    ...
    for(int idigi = 0; idigi < WCDC_hits->entries(); idigi++) {
      ...
      for(G4int id = 0; id < (*WCDC_hits)[idigi]->GetTotalPe(); id++){
        ...
        if (trackID>0) // skip noise hit
        {
          ...
          //std::cout<< trj->GetPhotonRayScatter() << std::endl;
	  // hit_photon_RamScatter = trj->GetPhotonRamScatter();
          //std::cout<< trj->GetPhotonRamScatter() << std::endl;
          hit_photon_MieScatter = trj->GetPhotonMieScatter();
          //std::cout<< trj->GetPhotonMieScatter() << std::endl;
    ...
    for(int idigi = 0; idigi < WCDC_hits->entries(); idigi++) {
      ...
      for(G4int id = 0; id < (*WCDC_hits)[idigi]->GetTotalPe(); id++){
        ...
        if (trackID>0) // skip noise hit
        {
          WCSimTrajectory* trj = (WCSimTrajectory*)(*TC)[trajMap[trackID]];
          hit_photon_RayScatter = trj->GetPhotonRayScatter();
          //std::cout << "Rayleigh Number: "<< trj->GetPhotonRayScatter() << std::endl;
          //hit_photon_RamScatter = trj->GetPhotonRamScatter();
          //std::cout << "Raman Number: " << trj->GetPhotonRamScatter() << std::endl;
          hit_photon_MieScatter = trj->GetPhotonMieScatter();
          //std::cout << "Mie Number: " << trj->GetPhotonMieScatter() << std::endl;
          hit_photon_reflection = trj->GetPhotonReflection();
