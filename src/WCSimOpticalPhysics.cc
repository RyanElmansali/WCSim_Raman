  ...
  for ( G4int i=0; i<ksNoProcess; i++ ) {
    ...
    if ( ! fProcessUse[i] ) {
      ...
      if ( i == ksCerenkov ) {
        ...
        if ( fProcessTrackSecondariesFirst[ksCerenkov] ) {
      ...
      if ( i == ksScintillation ) {
        ...
        if ( fProcessTrackSecondariesFirst[ksScintillation] ) {
          G4cout << "    Track secondaries first:  activated" << G4endl;
        }
        else {
          G4cout << "    Track secondaries first:  inactivated" << G4endl;
...
void WCSimOpticalPhysics::ConstructProcess()
{
  ...
  for ( G4int i=ksAbsorption; i<=ksWLS; i++ ) {
      if ( fProcessUse[i] ) {
...
#endif
  fScintillationProcess->SetTrackSecondariesFirst(fProcessTrackSecondariesFirst[ksScintillation]);
#if G4VERSION_NUMBER > 1030
  fScintillationProcess->SetStackPhotons(fScintillationStackPhotons);
#endif
  G4EmSaturation* emSaturation = G4LossTableManager::Instance()->EmSaturation();
  fScintillationProcess->AddSaturation(emSaturation);
  OpProcesses[ksScintillation] = fScintillationProcess;
  ...
  fCerenkovProcess->SetTrackSecondariesFirst(fProcessTrackSecondariesFirst[ksCerenkov]);
#if G4VERSION_NUMBER > 1030
  fCerenkovProcess->SetStackPhotons(fCerenkovStackPhotons);
#endif
  OpProcesses[ksCerenkov] = fCerenkovProcess;
  ...
  while( (*myParticleIterator)() ){
    ...
    if( fCerenkovProcess->IsApplicable(*particle) &&
        fProcessUse[ksCerenkov] ) {
          pManager->AddProcess(fCerenkovProcess);
          pManager->SetProcessOrdering(fCerenkovProcess,idxPostStep);
    }
    if( fScintillationProcess->IsApplicable(*particle) &&
        fProcessUse[ksScintillation] ) {
...
{
  fProcessTrackSecondariesFirst[ksCerenkov] = val;
  if (fCerenkovProcess) {
    fCerenkovProcess->
      SetTrackSecondariesFirst(fProcessTrackSecondariesFirst[ksCerenkov]);
  }
}
...
{
  fProcessTrackSecondariesFirst[ksScintillation] = val;
  if (fScintillationProcess) {
    fScintillationProcess->
      SetTrackSecondariesFirst(fProcessTrackSecondariesFirst[ksScintillation]);
  }
}
