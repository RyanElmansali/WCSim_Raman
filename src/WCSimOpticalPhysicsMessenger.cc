...
WCSimOpticalPhysicsMessenger::WCSimOpticalPhysicsMessenger(
  ...
  : G4UImessenger(),
    ...
    fSelectedProcessIndex(ksNoProcess),
...
/// Apply command to the associated object.
  if (command == fActivateProcessCmd) {
    ...
    G4ExceptionDescription function_msg;
    function_msg << "function entered";
    ...
    is >> pn >> flag;
    if  ( pn == "Cerenkov" )        {
        fSelectedProcessIndex = ksCerenkov;
    } else if ( pn == "Scintillation" ) {
        fSelectedProcessIndex = ksScintillation;
    } else if ( pn == "OpAbsorption" )  {
        fSelectedProcessIndex = ksAbsorption;
    } else if ( pn == "OpRayleigh" )    {
        fSelectedProcessIndex = ksRayleigh;
    } else if ( pn == "OpRaman" )    {
        fSelectedProcessIndex = ksRaman;
        
        //std::cout << "Raman activated" << std::endl;
        //G4ExceptionDescription raman_msg;
        //raman_msg << "Raman activated";
        
    } else if ( pn == "OpMieHG" )       {
        fSelectedProcessIndex = ksMieHG;
    } else if ( pn == "OpBoundary" )    {
        fSelectedProcessIndex = ksBoundary;
    } else if ( pn == "OpWLS" )         {
        fSelectedProcessIndex = ksWLS;
    } else {
