...
void WCSimTrajectory::AppendStep(const G4Step* aStep)
{
  ...
  if (PDGEncoding==0)
  {
    ...
    {
      if ( pds->GetProcessSubType() == fsOpRayleigh )
      {
        AddPhotonRayScatter(1);
        //std::cout <<"RAYLEIGH COUNTED"<< std::endl;
      }
      else if ( pds->GetProcessSubType() == fsOpRaman )
      {
        AddPhotonRamScatter(1);
	// std::cout <<"RAMAN COUNTED"<< std::endl;
      }
      else if ( pds->GetProcessSubType() == fsOpMieHG )
      {
